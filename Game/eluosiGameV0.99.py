...
pygame.mixer.init()
# 引入像素字体文件
font_path = '/Font_Press_Start_2P/Press_Start_2P/PressStart2P-Regular.ttf'  # 请替换为你的字体文件路径
# 在初始化部分加载字体
font = pygame.font.Font(font_path, 32)  # 第二个参数是字号

# 加载并播放背景音乐
pygame.mixer.music.load('/Game/Tetris.MP3')  # 替换为您的音乐文件路径
# pygame.mixer.music.play(-1)  # 循环播放
line_clear_sound = pygame.mixer.Sound('/Game/TerisEliminate.mp3')  # 替换为您的音效文件路径
        ...
        for x, cell in enumerate(row):
            if cell:
                ...
                pygame.draw.rect(surface, BLACK,
                                 (display_x + x * BLOCK_SIZE, display_y + y * BLOCK_SIZE, BLOCK_SIZE, BLOCK_SIZE))
                pygame.draw.rect(surface, LIGHT_GREY,
                                 (display_x + x * BLOCK_SIZE + 2, display_y + y * BLOCK_SIZE + 2, BLOCK_SIZE - 6, BLOCK_SIZE - 6))
                pygame.draw.rect(surface, BLACK,
                                 (display_x + x * BLOCK_SIZE + 5, display_y + y * BLOCK_SIZE + 5, BLOCK_SIZE - 12, BLOCK_SIZE - 12))
...
#             lines_removed += 1
#             i = len(grid) - 1  # 重新开始检查，因为grid的长度发生了变化
#         else:
#             i -= 1
#     if lines_removed > 0:
#         # 基本得分
...
#     total_lines_removed += lines_removed # 更新显示方块消除总行数
...
# 闪烁效果的函数
# 闪烁多行效果的函数
def flash_lines_animation(screen, grid, lines, color_flash, color_original, block_size, game_x, game_y, flash_times=2, flash_duration=100):
    for _ in range(flash_times):
        # 为所有需要闪烁的行绘制闪烁色
        for y in lines:
            for x in range(len(grid[0])):
                pygame.draw.rect(screen, color_flash, (game_x + x * block_size, game_y + y * block_size, block_size, block_size))
        pygame.display.update()
        pygame.time.delay(flash_duration)
        # 还原所有需要闪烁的行的原色
        for y in lines:
            for x in range(len(grid[0])):
                pygame.draw.rect(screen, color_original, (game_x + x * block_size, game_y + y * block_size, block_size, block_size))
        pygame.display.update()
        pygame.time.delay(flash_duration)
...
while running:
    ...
    pygame.draw.rect(screen, info_box_color2,
                     (score_box_x + 2, score_box_y + 2, score_box_width - 5, score_box_height - 5), border_radius = corner_radius)
    pygame.draw.rect(screen, info_box_color3,
                     (score_box_x + 5, score_box_y + 5, score_box_width - 10, score_box_height - 10), border_radius = corner_radius)
    ...
    # blit分数值到屏幕，标题下方
    screen.blit(score_value_text, (score_box_x + padding + 70, score_box_y + score_title_text.get_height() + padding + 8))
